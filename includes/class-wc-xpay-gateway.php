...
if ( !class_exists( 'WC_Xpay_Gateway' ) ):
	class WC_Xpay_Gateway extends WC_Payment_Gateway_Xpay
	{
		...
		public function __construct( )
		{
			...
			// Actions.
			add_action( 'valid_xpay_ipn_request', array(
				 $this,
				'successful_request'
			) );
			...
			add_action( 'woocommerce_xpay_metabox', array(
				 $this,
				'woocommerce_xpay_metabox'
			) );
			...
			if ( is_admin() && isset( $_GET[ 'section' ] ) && $this->id == $_GET[ 'section' ] ) {
				...
				if ( $t1 != 'testing' ) {
					add_action( 'admin_notices', array(
						 $this,
						'directory_includes_nowrite'
					) );
				}
				if ( $t3 != 'testing' ) {
					add_action( 'admin_notices', array(
						 $this,
						'directory_logs_nowrite'
					) );
				}
		...
		public function payment_fields( )
		{
			...
			if ( !$api->isApiKeyValid() ) {
				echo __( 'Xpay no esta disponible.', 'woocommerce-xpay' );
				return;
			}
			...
			echo $this->description . '<br /><b>' . __( 'Seleccione una moneda:', 'woocommerce-xpay' ) . '</b><br /><ul>';
			$first = true;
		...
		public function get_convertion_rate( $currency_org, $currency_dst )
		{
			...
			if ( $this->convertion_option == 'live-rates' ) {
				...
				foreach ( $api_arr as $fields ) {
					if ( strlen( $fields[ 'currency' ] ) == 7 && preg_match( '/[A-Z0-9]{3}\/[A-Z0-9]{3}/', $fields[ 'currency' ] ) ) {
			...
			} else if ( ( $this->convertion_option == 'dicom' || $this->convertion_option == 'promedio' ) && ( in_array( $currency_org, array(
					 'VES',
					'VEF',
					'VEB'
				) ) || in_array( $currency_dst, array(
					 'VES',
					'VEF',
					'VEB'
				) ) ) ) {
		...
		{
			$available = false;
			$available = ( 'yes' == $this->settings[ 'enabled' ] ) && !empty( self::$client_secret ) && $this->using_supported_currency();
			if ( $available ) {
				$api = new XpayLib( self::get_client_secret() );
				if ( !$api->isApiKeyValid() ) {
					$available = false;
		...
		public function generate_form( $order_id )
		{
			...
			$payment = self::get_metadata( $order_id, 'payment_data' );
			if ( !$payment ) {
			...
			if ( $payment && $payment[ 'status' ] == 'sending' ) {
				self::set_metadata( $order_id, 'payment_data', $payment );
				self::set_metadata( $order_id, 'string_amount_to_change', $payment[ 'string_amount_to_change' ] );
				self::set_metadata( $order_id, 'string_amount_to_paid', $payment[ 'string_amount_to_paid' ] );
				self::set_metadata( $order_id, 'string_crypto_amount_to_commerce', $payment[ 'string_crypto_amount_to_commerce' ] );
		...
		public function checkIpn( $order_id )
		{
			if ( (int) $order_id > 0 ) {
				...
				self::set_metadata( $order_id, 'payment_data', $input );
				self::set_metadata( $order_id, 'status', $input[ 'status' ] );
				self::debug( 'Actual transaction: ' . self::pL( $input, true ) );
		...
		{
			wp_enqueue_script( 'wc-xpayjs', plugins_url( 'woocommerce-xpay/xpay_script.js', basename( __FILE__ ) ), array(
				 'jquery'
			), WC_Xpay::VERSION, true );
			wp_localize_script( 'wc-xpayjs', 'wc_xpay_context', array(
				 'home_url' => home_url(),
				'messages' => array(
		...
		static public function set_metadata( $order_id, $key, $value )
		{
			...
			$exists                                         = $wpdb->get_var( $wpdb->prepare( "SELECT `id` FROM
                    $table_name
                WHERE
                        `order_id` = %d
                    AND
                        `key` = '%s'
                LIMIT 1", (int) $order_id, $key ) );
			if ( $exists ) {
		...
		static public function get_metadata( $order_id, $key )
		{
			...
			$table_name = $wpdb->prefix . 'woo_xpay';
			$data       = $wpdb->get_var( $wpdb->prepare( "SELECT `data` FROM
                    $table_name
                WHERE
                        `order_id` = %d
                    AND
		...
		public static function showLabelMetabox( $order_id, $id, $text, $is_price = false )
		{
            ...
            <tr>
				...
				<td><?php echo $is_price ? wc_price( $data ) : $data; ?></td>
            </tr>
            <?php
		}
		/**
		...
		{
			echo '<div class="error"><p><strong>' . __( 'Xpay', 'woocommerce-xpay' ) . '</strong>: ' . __( 'Ingresa un token valido.', 'woocommerce-xpay' ) . '</p></div>';
		}
		...
		{
			echo '<div class="error"><p><strong>' . __( 'Xpay Desactivado', 'woocommerce-xpay' ) . '</strong>: ' . sprintf( __( 'El token invalido. %s', 'woocommerce-xpay' ), '<a href="' . $this->admin_url() . '">' . __( 'Click aqui para configurarlo!', 'woocommerce-xpay' ) . '</a>' ) . '</p></div>';
		}
		/**
